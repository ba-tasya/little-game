...
public class DoorTopdown : MonoBehaviour
{
    ...
    private void SwitchToPlatformer()
    {
        // Переключаем режим управления на topdown
        CharacterController characterController = player.GetComponent<CharacterController>();
        characterController.currentMode = CharacterController.ControlMode.Platformer;

        // Изменяем спрайт персонажа на для topdown режима
        characterController.GetComponent<SpriteRenderer>().sprite = characterController.platformerSprite;
    }
}
